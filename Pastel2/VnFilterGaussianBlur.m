...
@implementation VnFilterGaussianBlur
...
- (id)init;
{
    ...
    NSString *currentGaussianBlurFragmentShader = [[self class] fragmentShaderForOptimizedBlurOfRadius:4 sigma:2.0 opacity:1.0];
...
+ (NSString *)fragmentShaderForOptimizedBlurOfRadius:(NSUInteger)blurRadius sigma:(CGFloat)sigma opacity:(float)opacity;
{
    if (blurRadius < 1)
    {
        return kGPUImagePassthroughFragmentShaderString;
    ...
    NSMutableString *shaderString = [[NSMutableString alloc] init];
    [shaderString appendString:kVnImageFilterFragmentShaderString];
...
- (void)createShader
{
    ...
    NSString *newGaussianBlurFragmentShader = [[self class] fragmentShaderForOptimizedBlurOfRadius:calculatedSampleRadius sigma:_blurRadiusInPixels opacity:self.opacity];
