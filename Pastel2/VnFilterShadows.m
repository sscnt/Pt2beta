...
(
 ...
 void main()
 {
     ...
     if(shadows > 0.0){
         rs.rgb = log(exp(pixel.rgb * (1.0 + shadows * 1.3)));
     }else{
         rs.rgb = log(exp(pixel.rgb * (1.0 + shadows)));
     }
     mediump float lum = pixel.r * 0.299 + pixel.g * 0.587 + pixel.b * 0.114;
     mediump float w = texture2D(toneCurveTexture, vec2(lum, 0.0)).r;
     
     gl_FragColor = blendWithBlendingMode(pixel, vec4(rs.r, rs.g, rs.b, topLayerOpacity * w), blendingMode);
...
- (id)init;
{
    if (!(self = [self initWithACV:@"cq002"]))
    {
		return nil;
    }
