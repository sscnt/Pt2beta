...
@interface VnVignetteImageACVFile : NSObject{
    short version;
    short totalCurves;
...
unsigned short vnVignetteInt16WithBytes(Byte* bytes);
@end

@implementation VnVignetteImageACVFile
...
- (id) initWithACVFileData:(NSData *)data {
    ...
    if (self != nil)
    {
        ...
        version        = vnVignetteInt16WithBytes(rawBytes);
        rawBytes+=2;
        
        totalCurves    = vnVignetteInt16WithBytes(rawBytes);
        rawBytes+=2;
        ...
        {
            unsigned short pointCount = vnVignetteInt16WithBytes(rawBytes);
            rawBytes+=2;
            ...
            {
                unsigned short y = vnVignetteInt16WithBytes(rawBytes);
                rawBytes+=2;
                unsigned short x = vnVignetteInt16WithBytes(rawBytes);
                rawBytes+=2;
...
NSString *const kVnFilterVignetteFragmentShaderString = SHADER_STRING
(
...
@implementation VnFilterVignette
...
- (id)initWithACVData:(NSData *)data {
    if (!(self = [super initWithFragmentShaderFromString:kVnFilterVignetteFragmentShaderString]))
    {
		return nil;
    }
...
{
    NSData* fileData = [NSData dataWithContentsOfURL:curveFileURL];
    VnVignetteImageACVFile *curve = [[VnVignetteImageACVFile alloc] initWithACVFileData:fileData];
...
{
    [self setFloat:multiplierX forUniformName:@"multiplierX"];
}
